...
object subtyping {
  ...
  def observeCounter (f : Counter => Unit) : Int = {
    ...
    class firstCounter extends Counter {
    	private var p = 0;
    	override def decrement () = {p = p + 1}
    	override def increment () = {p = p + 1 }
    	override def get () : Int = p
    }
	 val x = new firstCounter();
	 f(x);
	 x.get();
  }
  ...
  def observeCounterList (f : List[Counter] => Unit) : List[Int] = {
    ...
    val q = new ObserveCounter();
    f(List(m, n, q));

 	 list = List(m.get(), n.get(), q.get());
 	 list;
  }
  ...
  def observeCounterArray (f : Array[Counter] => Unit) : Array[Int] = {
    // TODO: Provide definition here.
    class observeArray extends Counter {
    	private var n = 0;
    	override def decrement () = {n = n + 1}
    	override def increment () = {n = n + 1 }
    	override def get () : Int = n
    }

    var cArrayList : Array[Int] = new Array[Int] (3);
    val p = new observeArray();
    val q = new observeArray();
    val r = new observeArray();
    f(Array(p, q, r));

 	 cArrayList = Array(p.get(), q.get(), r.get());
 	 cArrayList;
  }
}
